...
VOID USBD_CDC_ACM_Activate(VOID *cdc_acm_instance)
{
  /* USER CODE BEGIN USBD_CDC_ACM_Activate */
	...
	if (HAL_UART_Receive_IT(uart_handler, (uint8_t*) UserTxBufferFS, 1)
			!= HAL_OK) {
	...
	while (1) {
      ...
      cdc_acm -> ux_slave_class_cdc_acm_transmission_status = UX_FALSE;
			...
			if (actual_length != 0) {
				...
				if (HAL_UART_Transmit_DMA(uart_handler,
						(uint8_t*) UserRxBufferFS, actual_length) != HAL_OK) {
					Error_Handler();
				}
...
VOID usbx_cdc_acm_write_thread_entry(ULONG thread_input) {
    ...
    cdc_acm -> ux_slave_class_cdc_acm_transmission_status = UX_FALSE;
		...
		if (UserTxBufPtrOut != UserTxBufPtrIn) {
			...
			if (ux_device_class_cdc_acm_write(cdc_acm,
					(UCHAR*) (&UserTxBufferFS[buffptr]), buffsize,
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart) {
	...
	if (tx_event_flags_set(&EventFlag, TX_NEW_TRANSMITTED_DATA,
	TX_OR) != TX_SUCCESS) {
		Error_Handler();
	}
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	...
	if (tx_event_flags_set(&EventFlag, RX_NEW_RECEIVED_DATA,
	TX_OR) != TX_SUCCESS) {
		Error_Handler();
	}
	...
	if (HAL_UART_Receive_IT(uart_handler,
			(uint8_t*) UserTxBufferFS + UserTxBufPtrIn, 1) != HAL_OK) {
		/* Transfer error in reception process */
		Error_Handler();
	}
...
static VOID USBD_CDC_VCP_Config(
	...
	HAL_UART_Receive_IT(uart_handler,
			(uint8_t*) (UserTxBufferFS + UserTxBufPtrIn), 1);
}
